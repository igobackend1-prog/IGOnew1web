...
for (const [key, value] of Object.entries(replacements)) {
  if (content.includes(key)) {
    // Only replace the FIRST occurrence in case there are duplicates we already caught
    // Or rather, we want to see which ones fail.
    // Actually, if a line has `name: '...',` BUT it's part of the items list inside the subcategory, it might fail.
    // For example, `{ name: 'Polyhouse and Netting' }` vs `name: 'Polyhouse and Netting',`
    // Oh! The products have `items: [{ name: 'Polyhouse Sheets' }]`. But my keys have `,`.
  } else {
    // try removing the comma to see if that works
    const noCommaKey = key.replace(',', '');
    if (content.includes(noCommaKey)) {
        content = content.replace(noCommaKey, value);
        console.log(`Replaced without comma: ${noCommaKey}`);
    } else {
        console.log(`Failed totally: ${key}`);
    }
  }
}

// Add a generic function to forcefully inject if it failed
